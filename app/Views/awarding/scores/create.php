    ...
    <div class="card">
      ...
      <div class="card-body">
        ...
        <div class="alert alert-info">
          <i class="fas fa-info-circle"></i> Semua website sudah memiliki penilaian untuk periode ini, atau belum ada website yang terdaftar.
        </div>
        <a href="<?= base_url('admin/awarding/scores?period_id=' . $selectedPeriod['id']) ?>" class="btn btn-secondary">
          <i class="fas fa-arrow-left"></i> Kembali
        </a>
        <?php else: ?>
        <form action="<?= base_url('admin/awarding/scores/store') ?>" method="post" id="scoreForm">
          <?= csrf_field() ?>
          <input type="hidden" name="awarding_period_id" value="<?= $selectedPeriod['id'] ?>">

          <!-- Website Selection -->
          <div class="form-group">
            <label for="website_id">Pilih Website <span class="text-danger">*</span></label>
            <select class="form-control" id="website_id" name="website_id" required>
              ...
              <option value="<?= $website['id'] ?>" <?= old('website_id') == $website['id'] ? 'selected' : '' ?>>
                ...
                <?php if (!empty($website['category'])): ?>[<?= ucfirst($website['category']) ?>]<?php endif; ?>
              </option>
              <?php endforeach; ?>
            </select>
          </div>
          ...
          <div class="card bg-light mb-4" id="importSection">
            <div class="card-body">
              ...
              <div class="form-row align-items-end">
                ...
                <div class="form-group col-md-3 mb-0">
                  <button type="button" class="btn btn-info" id="btnImport" disabled>
          ...
          <div class="form-row">
            <div class="form-group col-md-4">
              ...
              <input type="number" class="form-control" id="total_posts" name="total_posts"
                     value="<?= old('total_posts', 0) ?>" min="0" required>
            </div>
          </div>
          ...
          <div class="row">
            <?php $colCount = 0; foreach ($elements as $field => $label): ?>
          ...
          <!-- Hidden import flag -->
          <input type="hidden" name="is_imported" id="is_imported" value="0">

          <!-- Notes -->
          <hr>
          <div class="form-group">
            <label for="notes">Catatan</label>
            <textarea class="form-control" id="notes" name="notes" rows="2" placeholder="Catatan tambahan (opsional)"><?= old('notes') ?></textarea>
          </div>

          <div class="form-group text-right">
            <a href="<?= base_url('admin/awarding/scores?period_id=' . $selectedPeriod['id']) ?>" class="btn btn-secondary mr-1">Batal</a>
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function toggleImportBtn() {
    btnImport.disabled = !(websiteSelect.value && perfPeriodSelect.value);
  }
  websiteSelect.addEventListener('change', toggleImportBtn);
  perfPeriodSelect.addEventListener('change', toggleImportBtn);
  toggleImportBtn();

  btnImport.addEventListener('click', function() {
    const websiteId = websiteSelect.value;
    const perfPeriodId = perfPeriodSelect.value;
    if (!websiteId || !perfPeriodId) return;
